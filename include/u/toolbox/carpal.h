...
/**
    ...
    \code
    Jan 15 12:27:27 tho-2 ./carp[14425]: [dbg][14425:main.c:20:f] (fp = fopen("my.txt", "w")) ==
   ((void *)0) [errno: 13, Permission denied] \endcode
...
/* nop_ macros */
#define nop_return_if(expr, err)                                                                   \
  do {                                                                                             \
    if (expr)                                                                                      \
      return err;                                                                                  \
  } while (0)
#define nop_err_if(expr)                                                                           \
  do {                                                                                             \
    if (expr)                                                                                      \
      goto err;                                                                                    \
  } while (0)
#define nop_goto_if(expr, gt)                                                                      \
  do {                                                                                             \
    if (expr)                                                                                      \
      goto gt;                                                                                     \
  } while (0)
...
#define con_return_ifm(expr, err, ...)  msg_return_ifm(con_, expr, err, __VA_ARGS__)
#define con_return_sifm(expr, err, ...) msg_return_sifm(con_, expr, err, __VA_ARGS__)
...
#define emerg_return_ifm(expr, err, ...)  msg_return_ifm(emerg_, expr, err, __VA_ARGS__)
#define emerg_return_sifm(expr, err, ...) msg_return_sifm(emerg_, expr, err, __VA_ARGS__)
...
#define alert_return_ifm(expr, err, ...)  msg_return_ifm(alert_, expr, err, __VA_ARGS__)
#define alert_return_sifm(expr, err, ...) msg_return_sifm(alert_, expr, err, __VA_ARGS__)
...
#define crit_return_ifm(expr, err, ...)  msg_return_ifm(crit_, expr, err, __VA_ARGS__)
#define crit_return_sifm(expr, err, ...) msg_return_sifm(crit_, expr, err, __VA_ARGS__)
...
#define err_return_ifm(expr, err, ...)  msg_return_ifm(err_, expr, err, __VA_ARGS__)
#define err_return_sifm(expr, err, ...) msg_return_sifm(err_, expr, err, __VA_ARGS__)
...
#define warn_return_ifm(expr, err, ...)  msg_return_ifm(warn_, expr, err, __VA_ARGS__)
#define warn_return_sifm(expr, err, ...) msg_return_sifm(warn_, expr, err, __VA_ARGS__)
...
#define notice_return_ifm(expr, err, ...)  msg_return_ifm(notice_, expr, err, __VA_ARGS__)
#define notice_return_sifm(expr, err, ...) msg_return_sifm(notice_, expr, err, __VA_ARGS__)

#define notice_err_if(expr)        msg_err_if(notice_, expr)
#define notice_err_sif(expr)       msg_err_sif(notice_, expr)
#define notice_err_ifm(expr, ...)  msg_err_ifm(notice_, expr, __VA_ARGS__)
#define notice_err_sifm(expr, ...) msg_err_sifm(notice_, expr, __VA_ARGS__)
#define notice_err_rcif(expr, ...) msg_err_rcif(notice_, expr, __VA_ARGS__)
...
#define info_return_ifm(expr, err, ...)  msg_return_ifm(info_, expr, err, __VA_ARGS__)
#define info_return_sifm(expr, err, ...) msg_return_sifm(info_, expr, err, __VA_ARGS__)
...
#  define dbg_return_ifm(expr, err, ...)  msg_return_ifm(dbg_, expr, err, __VA_ARGS__)
#  define dbg_return_sifm(expr, err, ...) msg_return_sifm(dbg_, expr, err, __VA_ARGS__)
...
#  define TIMER_ON                                                                                 \
    time_t _t_beg = time(0), _t_prev = _t_beg, _t_now;                                             \
    int _t_step = 0
#  define TIMER_STEP                                                                               \
    do {                                                                                           \
      _t_now = time(0);                                                                            \
      dbg("  step %u: %u s (delta: %u s)", _t_step++, _t_now - _t_beg, _t_now - _t_prev);          \
      _t_prev = _t_now;                                                                            \
    } while (0)
...
#  define dbg_return_ifm(expr, err, ...)  dbg_return_if(expr, err);
#  define dbg_return_sifm(expr, err, ...) dbg_return_if(expr, err);
