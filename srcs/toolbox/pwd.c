...
/* u_pwd_rec_t */
static int
    u_pwd_rec_new(const char* user, const char* pass, const char* opaque, u_pwd_rec_t** prec);
static int u_pwd_retr_mem(u_pwd_t* pwd, const char* user, u_pwd_rec_t** prec);
static int u_pwd_retr_res(u_pwd_t* pwd, const char* user, u_pwd_rec_t** prec);
...
static int __file_notify(const char* path, time_t last_update, time_t* pnew_update);
...
static int u_pwd_retr_res(u_pwd_t* pwd, const char* user, u_pwd_rec_t** prec) {
  ...
  dbg_err_ifm(u_strtok(ln, ":", &toks, &ntoks) || ntoks < 2, "bad syntax at line %zu (%s)", lc, ln);

  /* create new record */
  dbg_err_if(u_pwd_rec_new(toks[0], toks[1], ntoks < 3 ? NULL : toks[2], &rec));
...
static int u_pwd_retr_mem(u_pwd_t* pwd, const char* user, u_pwd_rec_t** prec) {
  ...
  dbg_ifb(u_pwd_need_reload(pwd)) u_warn("error reloading master pwd file: using stale cache");
...
static int u_pwd_db_load(u_pwd_t* pwd) {
  ...
  for (lc = 1; pwd->cb_load(ln, sizeof ln, pwd->res_handler) != NULL; lc++) {
    ...
    dbg_ifb(u_pwd_rec_new(toks[0], toks[1], ntoks < 3 ? NULL : toks[2], &rec)) {
