...
#define U_JSON_OBJ_NAME(jo) ((jo->parent != NULL) ? jo->fqn + strlen(p->fqn) : jo->fqn)

#define U_JSON_OBJ_IS_CONTAINER(jo)                                                                \
  ((jo->type == U_JSON_TYPE_OBJECT) || (jo->type == U_JSON_TYPE_ARRAY))

#define U_JSON_OBJ_IS_BOOL(jo) ((jo->type == U_JSON_TYPE_TRUE) || (jo->type == U_JSON_TYPE_FALSE))
...
/* Lexer misc stuff. */
static int u_json_match_seq(u_lexer_t* jl,
                            u_json_t* jo,
                            int type,
                            char first,
                            const char* rem,
...
static int u_json_do_parse(const char* json, u_json_t** pjo, char status[U_LEXER_ERR_SZ]);

/* Needed by hmap_easy* because we are storing pointer data not owned by the
 * hmap. */
static void nopf(void* dummy) {
  u_unused_args(dummy);
  return;
}

static int u_json_new_container(u_json_type_t type, const char* key, u_json_t** pjo);
static int u_json_new_atom(u_json_type_t type,
                           const char* key,
                           const char* val,
                           char check,
                           u_json_t** pjo);
...
int u_json_set_val_ex(u_json_t* jo, const char* val, char check) {
  ...
  if (check) {
    ...
    (void)u_snprintf(qval, sizeof qval, (jo->type == U_JSON_TYPE_STRING) ? "\"%s\"" : "%s", val);
...
int u_json_add(u_json_t* head, u_json_t* jo) {
  ...
  u_con("chld (%p): %s {%s} added at depth %u", jo, u_json_type_str(jo->type), jo->key, jo->depth);
  u_con("prnt (%p): %s {%s} at depth %u\n",
        head,
        u_json_type_str(head->type),
        head->key,
        head->depth);
...
void u_json_walk(u_json_t* jo,
                 int strategy,
                 size_t l,
                 void (*cb)(u_json_t*, size_t, void*),
                 void* cb_args) {
  dbg_return_if(strategy != U_JSON_WALK_PREORDER && strategy != U_JSON_WALK_POSTORDER, );
...
int u_json_new_bool(const char* key, char val, u_json_t** pjo) {
  /* No need to validate. */
  return u_json_new_atom(val ? U_JSON_TYPE_TRUE : U_JSON_TYPE_FALSE, key, NULL, 0, pjo);
}
...
static int u_json_match_seq(u_lexer_t* jl,
  ...
  if ((c = u_lexer_peek(jl)) != first) {
    U_LEXER_ERR(jl, "expect \'%c\', got %c at %s", first, c, u_lexer_lookahead(jl));
  }

  for (i = 0; i < rem_sz; i++) {
    U_LEXER_SKIP(jl, &c);
    if (c != *(rem + i)) {
      U_LEXER_ERR(jl, "expect \'%c\', got %c at %s", *(rem + i), c, u_lexer_lookahead(jl));
    }
  }
...
static int u_json_match_null(u_lexer_t* jl, u_json_t* jo) {
  return u_json_match_seq(jl, jo, U_JSON_TYPE_NULL, 'n', "ull", strlen("ull"));
}

static int u_json_match_true(u_lexer_t* jl, u_json_t* jo) {
  return u_json_match_seq(jl, jo, U_JSON_TYPE_TRUE, 't', "rue", strlen("rue"));
}

static int u_json_match_false(u_lexer_t* jl, u_json_t* jo) {
  return u_json_match_seq(jl, jo, U_JSON_TYPE_FALSE, 'f', "alse", strlen("alse"));
}
...
static void u_json_do_print(u_json_t* jo, size_t l, void* opaque) {
  ...
  switch (jo->type) {
    ...
    case U_JSON_TYPE_OBJECT:
      ...
      break;
    default:
      u_con("%*c %s %s : \'%s\'", l, ' ', u_json_type_str(jo->type), jo->key, jo->val);
      break;
  }
...
static int u_json_do_parse(const char* json, u_json_t** pjo, char status[U_LEXER_ERR_SZ]) {
  ...
  if (!u_lexer_eot(jl)) {
    u_warn("Unparsed trailing text \'%s\' at position %u", u_lexer_lookahead(jl), u_lexer_pos(jl));
  }
