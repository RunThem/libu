...
/* low level resolvers */
static int do_resolv(int rf(const char*, const char*, const char*, u_addrinfo_t* ai),
                     const char* host,
                     const char* port,
                     const char* path,
                     int family,
...
static int resolv_sun(const char* dummy1, const char* dummy2, const char* path, u_addrinfo_t* ai);
static int ai_resolv(const char* host,
                     const char* port,
                     const char* path,
                     int family,
                     int type,
...
#ifndef HAVE_GETADDRINFO
static int resolv_sin(const char* host, const char* port, const char* dummy, u_addrinfo_t* ai);
static int resolv_sin6(const char* host, const char* port, const char* dummy, u_addrinfo_t* ai);
static int resolv_port(const char* s_port, uint16_t* pin_port);
#endif /* !HAVE_GETADDRINFO */

/* socket creation horses */
static int do_sock(int f(struct sockaddr*, u_socklen_t, int, int, int, int, int, struct timeval*),
                   struct u_net_addr_s* a,
                   int backlog,
                   u_addrinfo_t** pai,
                   struct timeval* timeout);
static int do_csock(struct sockaddr* sad,
                    u_socklen_t sad_len,
                    int domain,
                    int type,
                    int protocol,
                    ...
                    int dummy,
                    struct timeval* timeout);
static int do_ssock(struct sockaddr* sad,
                    u_socklen_t sad_len,
                    int domain,
                    int type,
                    int protocol,
...
static inline int update_timeout(struct timeval* timeout, struct timeval* tstart);
...
static int do_sock(int wf(struct sockaddr*, u_socklen_t, int, int, int, int, int, struct timeval*),
                   struct u_net_addr_s* a,
                   int backlog,
                   u_addrinfo_t** pai,
                   struct timeval* timeout) {
  ...
  for (; ap != NULL; ap = ap->ai_next) {
    ...
    if ((sd = wf(ap->ai_addr,
      ...
      u_info("%s %s",
             (wf == do_ssock) ? "bind succeeded for" : "connect succeeded to",
             (ap->ai_family == AF_UNIX) ? ap->ai_canonname : u_sa_ntop(ap->ai_addr, h, sizeof h));
  ...
  if (sd == -1) {
    ...
    u_info("could not %s %s",
           (wf == do_ssock) ? "bind" : "connect to",
           (wai->ai_family == AF_UNIX) ? wai->ai_canonname : u_sa_ntop(wai->ai_addr, h, sizeof h));
...
#ifdef HAVE_SO_BROADCAST
    dbg_err_sif(u_setsockopt(s, SOL_SOCKET, SO_BROADCAST, &bc, sizeof bc) == -1);
#else
...
static int uri2addr(u_uri_t* u, u_net_scheme_map_t* m, u_net_addr_t* a) {
  ...
  return ai_resolv(u_uri_get_host(u),
                   u_uri_get_port(u),
                   u_uri_get_path(u),
                   m->addr_family,
                   m->sock_type,
                   m->proto,
                   (a->mode == U_NET_SSOCK) ? 1 : 0,
                   &a->addr);
}
...
static int resolv_sun(const char* dummy1, const char* dummy2, const char* path, u_addrinfo_t* ai) {
  ...
  dbg_err_ifm(u_strlcpy(sunix->sun_path, path, sizeof sunix->sun_path), "%s too long", path);
...
static int ai_resolv(const char* host,
   ...
   * to resolv_sun() */
  if (family == AF_UNIX) {
    return do_resolv(resolv_sun, NULL, NULL, path, family, type, proto, pai);
  }
...
static int ai_resolv(const char* host,
  ...
  switch (family) {
    case AF_INET:
      return do_resolv(resolv_sin, host, port, NULL, family, type, proto, pai);
    case AF_UNIX:
      return do_resolv(resolv_sun, NULL, NULL, path, family, type, proto, pai);
    case AF_INET6:
      return do_resolv(resolv_sin6, host, port, NULL, family, type, proto, pai);
    default:
...
static int resolv_sin(const char* host, const char* port, const char* dummy, u_addrinfo_t* ai) {
  ...
  if (!strcmp(host, "*")) {
    ...
    {
      dbg_err_ifm((hp = gethostbyname(host)) == NULL, "%s: %d", host, h_errno);
...
#  endif
  ...
  if (!strcmp(host, "*")) {
    ...
    if (!strchr(host, ':')) {
      ...
      dbg_err_ifm((hp = gethostbyname2(host, AF_INET6)) == NULL, "%s: %d", host, h_errno);
