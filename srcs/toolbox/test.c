...
#define TS_HANDLE(optr) (u_test_suite_t*)((char*)optr - offsetof(u_test_suite_t, o))

/* Get test case handle by its '.o(bject)' field. */
#define TC_HANDLE(optr) (u_test_case_t*)((char*)optr - offsetof(u_test_case_t, o))
...
static int u_test_obj_init(const char* id, u_test_what_t what, u_test_obj_t* to);
...
static int u_test_suite_report_txt(FILE* fp, u_test_suite_t* ts, u_test_rep_tag_t tag);
static int u_test_suite_report_xml(FILE* fp, u_test_suite_t* ts, u_test_rep_tag_t tag);
...
static int u_test_obj_ts_fmt(u_test_obj_t* to, char b[80], char e[80], char d[80]);
#ifdef HAVE_STRUCT_RUSAGE
static int u_test_case_rusage_fmt(u_test_case_t* tc, char uti[80], char sti[80]);
static int __timeval_fmt(struct timeval* tv, char t[80]);
#endif /* HAVE_STRUCT_RUSAGE */

/* Pre-cooked reporters. */
struct u_test_rep_s xml_reps = {u_test_report_xml, u_test_suite_report_xml, u_test_case_report_xml};

struct u_test_rep_s txt_reps = {u_test_report_txt, u_test_suite_report_txt, u_test_case_report_txt};
...
static void u_test_obj_print(char nindent, u_test_obj_t* to) {
  ...
  u_con("%*c=> [%s] %s", nindent, ' ', to->what == U_TEST_CASE_T ? "case" : "suite", to->id);
...
static int u_test_obj_sequencer(TO* h) {
  ...
  LIST_FOREACH(to, &h->objs, links) {
    dbg_err_ifm(!to->sequenced, "%s not sequenced: dependency loop !", to->id);
  }
        ...
        if (to->rank == r) {
          ...
          if (u_test_dep_failed(&to->deps)) {
            ...
            if (to->what == U_TEST_SUITE_T) {
              (void)u_test_suite_update_all_status(TS_HANDLE(to), U_TEST_SKIPPED);
            }
...
static int u_test_obj_ts_fmt(u_test_obj_t* to, char b[80], char e[80], char d[80]) {
  ...
  if (b) {
    dbg_if(strftime(b, 80, "%a %Y-%m-%d %H:%M:%S %Z", localtime(&to->start)) == 0);
  }

  if (e) {
    dbg_if(strftime(e, 80, "%a %Y-%m-%d %H:%M:%S %Z", localtime(&to->stop)) == 0);
  }
