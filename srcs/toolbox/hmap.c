...
struct u_hmap_pcy_s {
  ...
  int (*push)(u_hmap_t* hmap, u_hmap_o_t* obj, u_hmap_q_t** data);
...
static int __o_overwrite(u_hmap_t* hmap, u_hmap_o_t* o, u_hmap_o_t* obj, u_hmap_o_t** old);
...
static int __queue_push_count(u_hmap_t* hmap, u_hmap_o_t* obj, u_hmap_q_t** counts);
static int __queue_push_cmp(u_hmap_t* hmap, u_hmap_o_t* obj, u_hmap_q_t** counts);
...
static u_string_t* __f_str(u_hmap_o_t* obj) {
  ...
  key = (obj->hmap->opts->key_type == U_HMAP_OPTS_DATATYPE_STRING) ? (const char*)obj->key : "";
  val = (obj->hmap->opts->val_type == U_HMAP_OPTS_DATATYPE_STRING) ? (const char*)obj->val : "";
...
static int __opts_check(u_hmap_opts_t* opts) {
  ...
  dbg_err_if(opts->type != U_HMAP_TYPE_CHAIN && opts->type != U_HMAP_TYPE_LINEAR);
  ...
  /* in the hmap_easy interface, in case of pointer values (default),
     ...
     to handle them */
  dbg_err_ifm(opts->easy && opts->val_type == U_HMAP_OPTS_DATATYPE_POINTER && !opts->val_free_set,
              "value free function must be set for pointers!");

  dbg_err_ifm(opts->policy == U_HMAP_PCY_CUSTOM && opts->f_pcy_cmp == NULL,
...
int u_hmap_new(u_hmap_opts_t* opts, u_hmap_t** phmap) {
  ...
  dbg_return_sif((c = (u_hmap_t*)u_zalloc(sizeof(u_hmap_t))) == NULL, U_HMAP_ERR_FAIL);
  ...
  if (c->opts->f_str == NULL && (c->opts->key_type == U_HMAP_OPTS_DATATYPE_STRING ||
  ...
  dbg_err_sif((c->hmap = (u_hmap_e_t*)u_zalloc(sizeof(u_hmap_e_t) * c->size)) == NULL);
...
int u_hmap_put(u_hmap_t* hmap, u_hmap_o_t* obj, u_hmap_o_t** old) {
  ...
  if (hmap->sz >= hmap->threshold && hmap->opts->policy == U_HMAP_PCY_NONE) {
  ...
  if (hmap->opts->f_hash != &__f_hash && !(hmap->opts->options & U_HMAP_OPTS_HASH_STRONG)) {
    enum {
      MAX_INT = 20
    };
    char h[MAX_INT];
  ...
  switch (hmap->opts->type) {
    ...
    case U_HMAP_TYPE_LINEAR:
      ...
      for (; hash != last; hash = ((hash + 1) % hmap->size), x = &hmap->hmap[hash]) {
        if (LIST_EMPTY(x)) {
          LIST_INSERT_HEAD(x, obj, next);
          goto end;
        }
...
err:
  ...
  if (hmap->sz >= hmap->opts->max && hmap->opts->policy != U_HMAP_PCY_NONE) {
    u_dbg("Cache full - freeing according to policy '%s'", __pcy2str(hmap->opts->policy));
...
int u_hmap_opts_new(u_hmap_opts_t** opts) {
  ...
  dbg_err_sif((o = (u_hmap_opts_t*)u_zalloc(sizeof(u_hmap_opts_t))) == NULL);
...
void u_hmap_opts_dbg(u_hmap_opts_t* opts) {
  ...
  u_dbg("  buckets: %u (max: %u -- if discard policy != none)", opts->size, opts->max);
  u_dbg("  owns data: %s", (opts->options & U_HMAP_OPTS_OWNSDATA) ? "yes" : "no");
  ...
  u_dbg("  overwrite equal keys: %s", (opts->options & U_HMAP_OPTS_NO_OVERWRITE) ? "no" : "yes");
...
int u_hmap_opts_set_option(u_hmap_opts_t* opts, int option) {
  ...
  dbg_err_if((option != U_HMAP_OPTS_OWNSDATA && option != U_HMAP_OPTS_NO_OVERWRITE &&
              option != U_HMAP_OPTS_HASH_STRONG));

  dbg_err_if(opts->easy && option == U_HMAP_OPTS_OWNSDATA);
...
int u_hmap_opts_set_key_type(u_hmap_opts_t* opts, u_hmap_options_datatype_t type) {
  ...
  dbg_err_ifm(opts->easy, "cannot set data type when \"easy\" interface is in use");
...
static int __queue_pop_back(u_hmap_t* hmap, u_hmap_o_t** obj) {
  ...
  dbg_err_if((last = TAILQ_LAST(&hmap->pcy.queue, u_hmap_q_h_s)) == NULL);
...
static int __queue_push(u_hmap_t* hmap, u_hmap_o_t* obj, u_hmap_q_t** data) {
  ...
  if (*data == NULL) { /* no existing reference to queue entry -> push to head
...
static int __queue_push_count(u_hmap_t* hmap, u_hmap_o_t* obj, u_hmap_q_t** counts) {
  ...
  if (*counts == NULL) {
    ...
    *count = *count + 1;
    ...
    u_dbg("]");
    */
    for (q = TAILQ_NEXT(*counts, next); q && ((*count) >= *((int*)q->data));
         q = TAILQ_NEXT(q, next)) {
      ;
    }
...
static int __queue_push_cmp(u_hmap_t* hmap, u_hmap_o_t* obj, u_hmap_q_t** data) {
  u_hmap_q_t* new, *q;
  ...
  for (i = 0; i < hmap->size; ++i) {
    ...
    LIST_FOREACH(obj, &hmap->hmap[i], next) {
      if (hmap->opts->f_str == NULL) {
        ...
        dbg_err_if(u_string_append(s, u_string_c(st), u_string_len(st) - 1));
        u_string_free(st);
      }
    }
    u_dbg(u_string_c(s));
...
static u_hmap_q_t* __q_o_new(u_hmap_o_t* ho) {
  ...
  dbg_err_sif((qo = (u_hmap_q_t*)u_zalloc(sizeof(u_hmap_q_t))) == NULL);
...
static int __next_prime(size_t* prime, size_t sz, size_t* idx) {
  static size_t primes[] = {
      13,        19,        29,        41,         59,         79,         107,       149,
      197,       263,       347,       457,        599,        787,        1031,      1361,
      1777,      2333,      3037,      3967,       5167,       6719,       8737,      11369,
      14783,     19219,     24989,     32491,      42257,      54941,      71429,     92861,
      120721,    156941,    204047,    265271,     344857,     448321,     582821,    757693,
      985003,    1280519,   1664681,   2164111,    2813353,    3657361,    4754591,   6180989,
      8035301,   10445899,  13579681,  17653589,   22949669,   29834603,   38784989,  50420551,
      65546729,  85210757,  110774011, 144006217,  187208107,  243370577,  316381771, 411296309,
      534685237, 695090819, 903618083, 1174703521, 1527114613, 1837299131, 2147483647};
...
static int __resize(u_hmap_t* hmap) {
  ...
  dbg_err_if(__next_prime(&newopts->size, U_HMAP_RATE_RESIZE * newopts->size, &hmap->px));
