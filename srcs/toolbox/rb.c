...
static int create_mmap(size_t hint_sz, int opts, u_rb_t** prb) {
  ...
  rb->base = mmap(NULL, rb->sz << 1, PROT_NONE, MAP_ANON | MAP_PRIVATE, -1, 0);
  ...
  dbg_err_sif(mmap(rb->base, rb->sz, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, fd, 0) !=
              rb->base);
  ...
  dbg_err_sif(
      mmap(rb->base + rb->sz, rb->sz, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, fd, 0) !=
      rb->base + rb->sz);
